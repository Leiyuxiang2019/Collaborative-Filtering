...
def tf_idf(movies):
    ...
    for i in movies:
        for t in movies[i]:
            tempset.setdefault(t,0)
            tempset[t]+=1
    for i in tempset:  #求出所有词语数量
        num+=tempset[i]
    for i in tempset:
        tifset[i]=tempset[i]/num
        tifset[i]=tifset[i]*math.log(len(movies)/tempset[i])
    return tifset
...
def computesims(keys,moviename,movies):
 ...
 #推荐
def recommend(username, test,train,k,movies,n):
...
if __name__ == '__main__':
    ...
    for username in test:
           sse=sse+recommend(username, test, train, 10, movies, 10)
    print(sse)
